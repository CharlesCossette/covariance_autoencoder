import torch
from .utils import cholvec_to_covariance, tril_to_vec

def frobenius_error(
    cov1: torch.Tensor, cov2: torch.Tensor, reduction="mean", size=45, scaling_matrix = None
):
    """
    ...
    ----------
    cov1 : torch.Tensor
        Either batch of matrices or batch of lower triangular vectors
    cov2 : torch.Tensor
        Either batch of matrices or batch of lower triangular vectors
    ...
    if scaling_matrix is not None:
        cov1 = cov1 * scaling_matrix
        cov2 = cov2 * scaling_matrix

    e = torch.linalg.matrix_norm(cov1 - cov2) / torch.linalg.matrix_norm(cov2)
